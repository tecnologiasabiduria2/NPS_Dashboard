...
| B9 | 🟡 **Calendario de sesiones — v1 CONSTRUIDA (2026-06-23)** | 🟦 + 🟥 | **Salido de reunión 2026-06-22 (Diana + Garzón).** Diana pidió que los empresarios vean TODAS sus sesiones, no solo la próxima. **Construido:** vista cliente `/sessions` (lista cronológica agrupada por día, con "Anteriores"; botón Unirme → `/api/sessions/[id]/join`) + entrada en el sidebar; el admin ya tenía `/admin/sessions`. Se eligió **lista cronológica** (no grid de mes) para no sobre-construir. El "tipo" de sesión se muestra desde la columna **`live_sessions.tipo`** (5 valores: `inmersion_1`/`inmersion_2`/`mentoria`/`sala_gerencia`/`entrenamiento_comercial`) con labels legibles vía `lib/sessionTypes.ts` — tanto en `/sessions` (cliente) como en `/admin/sessions` (form + listado). **Sigue abierto:** (1) ¿quieren después vista de **grid mensual** estilo Google Calendar? (hoy es lista cronológica); (2) **link fijo recurrente** (sala de gerencia/entrenamiento) vs. **variable** (inmersión/mentoría) — hoy `zoom_url` es un solo campo (ver sección E). **Nuevo (2026-06-23): ventana de asistencia** en el join (margen `starts_at−10min` … `ends_at+15min`): fuera de ventana redirige a Zoom pero NO cuenta asistencia — el dato es honesto. Constantes ajustables en el route. |
