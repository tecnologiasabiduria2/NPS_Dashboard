...
| **6 — Área admin** | ✅ Completo | dashboard (KPIs+alertas), clients (lista), clients/[id] (detalle + editar acceso + **agregar notas de coaching**, 2026-06-21), clients/create (alta manual), map, nps (resultados), **content (carga/edición de lecciones con validación de fathom contra el Worker, A6 — 2026-06-23)**. |
...
| **8 — Deploy VPS** | ✅ **Completo (2026-06-22)** | **EN VIVO en `https://vip.sabiduriaempresarial.com`.** Droplet compartido `142.93.7.13` (NYC1). App en `/root/applications/ventra` (commit **`56990ab`** — incluye Bloque 3 + panel de contenido A6 + scaffold Stripe pausado), servicio **systemd** `ventra-platform.service` (puerto **3001**, `Restart=always`, `enabled`), reverse-proxy nginx `sites-enabled/ventra.conf` → 3001, **SSL Let's Encrypt** (certbot, auto-renueva, expira 2026-09-20) tras Cloudflare (zona en **Full strict**). No se usó PM2 (se copió el patrón systemd de `diana-dashboard-front`). Aditivo: WordPress/CRM/n8n/dashboards intactos. **Pendiente:** prueba visual en navegador (login real) + ajustar Site/Redirect URLs de Supabase Auth a producción (ver `PENDIENTES.md` B5). |
