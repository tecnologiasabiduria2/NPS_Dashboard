...
INSERT INTO live_sessions (product_id, title, starts_at, ends_at, zoom_url, is_published)
SELECT
  p.id,
  v.title,
  NOW() + v.starts_in,
...
FROM products p
JOIN (VALUES
  ('sabiduria', 'Beta · Sabiduría — Inmersión 1',    INTERVAL '2 days',  'https://zoom.us/j/BETA_SAB_1'),
  ('sabiduria', 'Beta · Sabiduría — Mentoría',        INTERVAL '9 days',  'https://zoom.us/j/BETA_SAB_2'),
  ('desafio',   'Beta · Desafío — Inmersión 1',       INTERVAL '3 days',  'https://zoom.us/j/BETA_DES_1'),
  ('desafio',   'Beta · Desafío — Sala de gerencia',  INTERVAL '5 days',  'https://zoom.us/j/BETA_DES_2'),
  ('workshop',  'Beta · Workshop — Sesión 1',         INTERVAL '4 days',  'https://zoom.us/j/BETA_WRK_1'),
  ('workshop',  'Beta · Workshop — Sesión 2',         INTERVAL '11 days', 'https://zoom.us/j/BETA_WRK_2')
) AS v(slug, title, starts_in, zoom) ON v.slug = p.slug
WHERE NOT EXISTS (
